...
FormGlobalFn.showAlert = function (msg, icon, title) {
  ...
  layer.alert(msg, { icon: icon, title: title || "提示" });
};
...
FormGlobalFn.showWindow4Url = function (url, width, height, title, yesCallback, noYes) {
	if(noYes){
		  return layer.open({
			    ...
			    type: 2,
			    content: url,
			    area: [width || (FormGlobalConf.isMobileTemp? "100%": "95%"), height || (FormGlobalConf.isMobileTemp? "100%": "95%")],
			    btn: [ "关闭"]
			  });
	}
	return layer.open({
...
FormGlobalFn.check["letter"] = function (str) {
  ...
  return { state: result, msg: "只能输入英文字母！" };
};
...
FormGlobalFn.check["integer"] = function (str) {
  ...
  return { state: result, msg: "只能输入数字" };
};
FormGlobalFn.check["email"] = function (str) {
  ...
  return { state: result, msg: "邮件格式不正确！如：xxx@163.com" };
};
FormGlobalFn.check["idcard"] = function (str) {
  ...
  return { state: result, msg: "邮编格式不正确！" };
};
FormGlobalFn.check["mobile"] = function (str) {
  ...
  return { state: result, msg: "手机号码有误，请重新输入！" };
};
...
FormGlobalFn.check["zip"] = function (code) {
  ...
  return { state: row.pass, msg: row.msg };
};
...
Array.prototype.delete = function (index) {
  if (index instanceof Array) {
    index.sort(function(a,b){return a>b?-1:1;});//倒序
    for (let i = 0; i < index.length; i++) {
      if(index[i] > -1){
...
function setRadio(name,value,context){
...
function getRadio(name,context){
	let value="";
	$("input[name='"+name+"']",context).each(function(){
        if($(this).is(':checked')){
            value=this.value;
...
function getCheckBoxs(name,context){
	let value="";
	$("input[name='"+name+"']",context).each(function(){
        if($(this).is(':checked')){
			if(value==""){
 ...
 * values=1,2,3,4
 */
function setCheckBoxs(name,values,context){
	if(values==null){
		values="";
	}
